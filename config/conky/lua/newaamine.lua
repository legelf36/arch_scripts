...
local cpus = { -- gauge DATA.
	...
	{ -- cpu5
		...
		graph_unit_angle = 2.7,
		graph_unit_thickness = 2.7,
		graph_bg_colour = 0x0060FF,
		graph_bg_alpha = 0.3,
		graph_fg_colour = 0x999966,
		graph_fg_alpha = 1,
		hand_fg_colour = 0xEF5A29,
		hand_fg_alpha = 1,
		shadow_colour = 0x111111,
...
local img_data = { --image DATA.
	{ -- Arcolinux logo
		...
		h = 50,
		rotation = 0,
		file = "/home/kim/.config/conky/images/Original-Arch-Linux/archlinux-logo-dark-scalable-green.png",
	},
	{ -- weather icon
		name = "weather",
		x = 165,
...
local function graphics_load(display) -- init gauge rings.
	...
	local function setup_rings(display, data)
		local str = ""
		local value = 0
...
local function text_load(display) -- Show text.
	...
	if (os.date("%M") % 2) == 0 and (os.date("%S") % 30) == 0 then
		ret_weather()
	end
	if (os.date("%M") % 10) == 0 and (os.date("%S") % 30) == 0 then
		cp_weather()
	end

	for i, v in pairs(txt_data) do
